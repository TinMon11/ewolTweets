...
const TweetItem = ({
  ...
  function onPublish(data) {
    if (data.body.length > 0) {
      ...
      postTweet(data.body, data.user).then(() => {
        setpublishLoading(false)
        setBodyValue("")
      })
    } else {
      alert("Debes escribir al menos un caracter")
    }
  }

  return (
    <div
      className={`tweet-container${bodyFocus   ? " focus" : ""} ${error ? " error" : ""
        }`}
    >
      ...
      <div className="tweet-footer">
        ...
        {createMode ? (
          ...
          >
            {!publishLoading ? "Publicar" : "Publicando"}
          </Button>
        ) : (
